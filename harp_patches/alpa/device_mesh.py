...
class LocalPhysicalDeviceMesh(PhysicalDeviceMesh):
    ...
    def __init__(self, devices: Sequence["Device"] = None):
        raise NotImplementedError("Harp donot support LocalPhysicalDeviceMesh")
...
class DeviceCluster:
    ...
    def _build_hetero_metadata(self, cluster_info):
        host_ids_2d = []
        gpu_types = []
        num_gpu_per_hosts = []
        for sub_cluster_info in cluster_info:
            gpu_type = sub_cluster_info["gpu_info"]
            host_ids = []
            gpus_per_host = None
            for idx, host_info in enumerate(self.host_info):
                gpu_type_keys = [
                    (key, value)
                    for key, value in host_info["Resources"].items()
                    if key.startswith("gpu-type")
                ]
                assert len(gpu_type_keys) == 1, (
                    f"Error in ray initialization, there are {len(gpu_type_keys)} "
                    f"gpu-type in host {idx}, with host_info: {host_info}")
                key, value = gpu_type_keys[0]
                host_gpu_type = key.split("-")[2]
                if host_gpu_type == gpu_type:
                    host_ids.append(idx)
                    if gpus_per_host is None:
                        gpus_per_host = int(value)
                    else:
                        assert gpus_per_host == int(value), (
                            f"Error in ray initialization, the gpu-type {gpu_type} "
                            f"has different number of gpus in host {idx}, "
                            f"with host_info: {host_info}")
            assert len(host_ids) > 0, (
                f"No hosts found for gpu-type {gpu_type}")
            host_ids_2d.append(host_ids)
            gpu_types.append(gpu_type)
            num_gpu_per_hosts.append(gpus_per_host)

        return host_ids_2d, gpu_types, num_gpu_per_hosts

    def _validate_homogeneous(self):
        gpu_type_set = set()
        gpus_per_host_set = set()
        for idx, host_info in enumerate(self.host_info):
            gpu_type_keys = [
                (key, value)
                for key, value in host_info["Resources"].items()
            ...
            assert len(gpu_type_keys) == 1, (
                ...
                f"gpu-type in host {idx}, with host_info: {host_info}")
            key, value = gpu_type_keys[0]
            gpu_type = key.split("-")[2]
            gpu_type_set.add(gpu_type)
            gpus_per_host_set.add(int(value))

        assert len(gpu_type_set) == 1, (
            f"Homogeneous mode requires a single GPU type, "
            f"but got {sorted(gpu_type_set)}")
        assert len(gpus_per_host_set) == 1, (
            f"Homogeneous mode requires the same number of GPUs per host, "
            f"but got {sorted(gpus_per_host_set)}")

    def _print_cluster_info(self, enable_hetero: bool):
        color_title = "\033[1;36m"
        color_header = "\033[1;33m"
        color_key = "\033[1;32m"
        color_reset = "\033[0m"

        mode = "heterogeneous" if enable_hetero else "homogeneous"
        print(
            f"{color_title}Ray DeviceCluster initialized ({mode} mode){color_reset}")
        print(f"{color_header}Cluster summary:{color_reset}")

        gpu_type_to_hosts = defaultdict(list)
        gpu_type_to_gpus_per_host = {}
        for host_idx, host_info in enumerate(self.host_info):
            gpu_type_keys = [
                (key, value)
                for key, value in host_info["Resources"].items()
                if key.startswith("gpu-type")
            ]
            if len(gpu_type_keys) != 1:
                continue
            key, value = gpu_type_keys[0]
            gpu_type = key.split("-")[2]
            gpu_type_to_hosts[gpu_type].append(self.host_ips[host_idx])
            if gpu_type not in gpu_type_to_gpus_per_host:
                gpu_type_to_gpus_per_host[gpu_type] = int(value)

        for gpu_type, host_ips in gpu_type_to_hosts.items():
            gpus_per_host = gpu_type_to_gpus_per_host.get(gpu_type, "?")
            print(
                f"  {color_key}{gpu_type}{color_reset}: "
                f"hosts={host_ips}, gpus_per_host={gpus_per_host}")
    ...
    @property
    def hetero_info(self):
        return (self.host_ids, self.gpu_types, self.num_gpu_per_hosts)    
    ...
    def get_heter_info_init(self):
        # Get sub-cluster info from Ray: host groups by GPU type.
        gpu_host_ids = []
        gpu_per_hosts = []
        gpu_types = []
        ...
        for host_idx, host_info in enumerate(self.host_info):
            ...
            assert len(gpu_type_keys) == 1, (
                f"Error in ray initialization, there are {len(gpu_type_keys)} "
                f"gpu-type in host {host_idx}, with host_info: {host_info}")

            key, value = gpu_type_keys[0]
            gpu_type = key.split("-")[2]
            if gpu_type in gpu_types:
                idx = gpu_types.index(gpu_type)
                assert gpu_per_hosts[idx] == int(value), (
                    f"Error in ray initialization, the gpu-type {gpu_type} "
                    f"has different number of gpus in host {host_idx}, "
                    f"with host_info: {host_info}")
                gpu_host_ids[idx].append(host_idx)
            else:
                gpu_types.append(gpu_type)
                gpu_per_hosts.append(int(value))
                gpu_host_ids.append([host_idx])

        # Reorder by host_gpu_info
        assert len(gpu_types) == len(self.host_gpu_info)
        reordered_gpu_host_ids = []
        reordered_gpu_per_hosts = []
        reordered_gpu_types = []
        for gpu_type in self.host_gpu_info:
            idx = gpu_types.index(gpu_type)
            reordered_gpu_types.append(gpu_types[idx])
            reordered_gpu_per_hosts.append(gpu_per_hosts[idx])
            reordered_gpu_host_ids.append(gpu_host_ids[idx])
        gpu_host_ids = reordered_gpu_host_ids
        gpu_per_hosts = reordered_gpu_per_hosts
        gpu_types = reordered_gpu_types


# Global runtime objects
global_cluster: DeviceCluster = None
global_virtual_physical_mesh: Union[VirtualPhysicalMesh, Sequence[VirtualPhysicalMesh]] = None
...
def init_global_cluster(cluster_info,
    ...
    #     global_virtual_physical_mesh = [global_cluster.get_virtual_physical_mesh(host_ids[i], 
    #                                                                              num_gpu_per_hosts[i]) 
...
def get_global_physical_mesh():
    raise NotImplementedError("Harp does not support global physical mesh.")

def set_global_physical_mesh(mesh):
    raise NotImplementedError("Harp does not support global physical mesh.")


def set_global_virtual_physical_mesh(mesh: Union[VirtualPhysicalMesh, Sequence[VirtualPhysicalMesh]]):
