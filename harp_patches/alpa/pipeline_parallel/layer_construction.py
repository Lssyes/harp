...
def cluster_jaxpr_by_cost(jaxpr_eqn: Jaxpr, layer_num: int, eps: float, costs,
                          cost_criteria):
    """Clusters the jaxpr by cost."""
    layer_num = int(layer_num)
    length = len(jaxpr_eqn)
    ...
    for q in range(layer_num, 0, -1):
        k = a_argmin[r, q]
        reversed_sliced_eqns.append(jaxpr_eqn[k:r])
        r = k
...
def _check_layer_structure(sliced_eqns: Sequence[Sequence[JaxprEqn]]):
    '''
    除了第一个和最后一个layer, 其他 layer 的结构必须一摸一样
    '''
    assert len(sliced_eqns) > 4, "The number of repeated layers must be greater than 2 in order to use zero redundancy mode."
    for i in range(1, len(sliced_eqns) - 2):
        if len(sliced_eqns[i]) != len(sliced_eqns[i + 1]):
            raise ValueError("The structure of each layer must be the same except for the first and last layers.")
        for j in range(len(sliced_eqns[i])):
            if _check_eqns_diff(sliced_eqns[i][j], sliced_eqns[i + 1][j]):
                raise ValueError("The structure of each layer must be the same except for the first and last layers.")

def _check_eqns_diff(eqn1, eqn2):
    '''
    if eqn1 and eqn2 are different, return True'''
    if eqn1.primitive != eqn2.primitive:
        return True
    if eqn1.params != eqn2.params:
        return True
    if len(eqn1.outvars) != len(eqn2.outvars):
        return True
    for var1, var2 in zip(eqn1.outvars, eqn2.outvars):
        if var1.aval != var2.aval:
            return True
    if len(eqn1.invars) != len(eqn2.invars):
        return True
    for var1, var2 in zip(eqn1.invars, eqn2.invars):
        if var1.aval != var2.aval:
            return True
    return False
...
def get_costs_from_index(costs, start, end):
    '''
    costs: (non_trivial, input_sizes, compute_costs)
        non_trivial: np.array of bool, non_trivial[l] is True if the l-th equation is non-trivial
        input_sizes: np.array of float, input_sizes[l, r] is the input size of the l-th to (r - 1)-th equation
        compute_costs: np.array of float, compute_costs[l] is the cost of the l-th equation
    start: int, start index
    end: int, end index
    '''
    non_trivial, input_sizes, compute_costs = costs
    non_trivial = non_trivial[start:end]
    input_sizes = input_sizes[start:end + 1, start:end + 1]  # 修改这里
    compute_costs = compute_costs[start:end]
    return non_trivial, input_sizes, compute_costs
    ...
    def wrapped(*args):
        ...
        if auto_layer_boundary:
            ...
            if global_config.enable_ZRP:
                # 这里只提供了 GPT 和 MoE model 的支持
                # TODO: 整理ZRP的代码，支持自动识别 repeated modules & unrepeated modules

                assert layer_num is not None, "This is layer_num in repeated modules."

                costs = get_layer_construction_costs(jaxpr,
                                                     cost_criteria=cost_criteria)
                sliced_eqns = slice_eqns_by_layer_boundary(jaxpr)
                _check_layer_structure(sliced_eqns)

                ## 将 sliced_eqns 分为 sliced_eqns[0] + sliced_eqns[1:-1] + sliced_eqns[-1]
                ## 其中 sliced_eqns[1:-1] 为 repeated modules

                sliced_eqns_in_repeat_modules = []
                eps_in_repeat = global_config.layer_construction_eps

                for i, repeat_layer in enumerate(sliced_eqns[1:-1]):
                    start = len(sliced_eqns[0]) + i*len(sliced_eqns[1]) + (i + 1) # 这里的 i + 1 是因为 boundary eqn
                    end = len(sliced_eqns[0]) + (i+1)*len(sliced_eqns[1]) + (i + 1)
                    cost_in_repeat = get_costs_from_index(costs, start, end)
                    sliced_eqns_in_repeat, _ = cluster_jaxpr_by_cost(repeat_layer,
                                                                     layer_num,
                                                                     eps_in_repeat,
                                                                     cost_in_repeat,
                                                                     cost_criteria=cost_criteria)
                    sliced_eqns_in_repeat_modules.append(sliced_eqns_in_repeat)
                ...
                sliced_eqns_tmp = [sliced_eqns[0]]
                layer_nums.append(1)
                layer_mark.append("layer_-1_-999_0")
                for j, eqn in enumerate(sliced_eqns_in_repeat_modules):
                    sliced_eqns_tmp += eqn
                    layer_nums.append(layer_num)
                    for k in range(layer_num):
                        layer_mark.append(f"layer_0_{j}_{k}")
                sliced_eqns_tmp += [sliced_eqns[-1]]
                layer_nums.append(1)
                layer_mark.append("layer_-2_-999_0")

                sliced_eqns = sliced_eqns_tmp

                # marked layer
                # i_j_k 表示 
                #   第 i 种 repeated module 的第 j 个 repeated module, 
                #   k 表示第 j 个 repeated module 中的第 k 个 sub-layer
                # 若为 non-repeated module, 则 i < 0, j = -1, k = sub-layer-id
                # layer_mark_ids = ["-1_-999_0", "0_0_0", "0_0_1", 
                #                         "0_1_0", "0_1_1",
                #                         "0_2_0", "0_2_1",
                #                         "0_3_0", "0_3_1",
                #                         "0_4_0", "0_4_1",
                #                         "0_5_0", "0_5_1", "-2_-1_0"]
                # layer_mark = [("layer" + layer_mark_id) for layer_mark_id in layer_mark_ids]
                
                global_config.set_sliced_layer_mark(layer_mark, layer_nums)
                ...
                costs = get_layer_construction_costs(jaxpr,
                                                     cost_criteria=cost_criteria)
                sliced_eqns, _ = cluster_jaxpr_by_cost(jaxpr.eqns,
                                                       layer_num,
                                                       eps,
                                                       costs,
                                                       cost_criteria=cost_criteria)
