...
def compile_pipeshard_executable_internal(
    ...
    if global_config.enable_Hetero:
        ...
        #     if vmesh.launched_physical_mesh_group is None:
        #         vmesh.get_physical_mesh_group([sliced_virtual_meshes[i]])
        # Intuitively, the code above is what we should do. However, the
        # downstream logic only accesses launched_physical_mesh_group. If we
        # launch multiple groups, it would require widespread changes.
        # Therefore, we bind the physical meshes of all sliced_virtual_meshes
        # to the first virtual_mesh:
        # virtual_mesh[0] <- sliced_virtual_meshes[0].get_physical_mesh()
        #                 <- sliced_virtual_meshes[1].get_physical_mesh()
        #                ...
        # This breaks the one-to-one mapping between virtual_mesh and
        # sliced_virtual_meshes, but minimizes code changes.
        virtual_mesh = virtual_mesh[0]
