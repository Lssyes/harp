...
class ManualSchedule(PipeDreamFlush):
    def __init__(self,
                 dependency,
                 meshes,
                 apply_grad_placement,
        ...
        # a decreasing list of number of microbatches to launch manually
        for i, num in enumerate(manual_schedule_strategy):
            assert num > 0 and num <= num_batch, f"Invalid manual_schedule_strategy: {manual_schedule_strategy}"
            if i>0:
                assert manual_schedule_strategy[i] < manual_schedule_strategy[i-1], f"manual_schedule_strategy should be decreasing. but {manual_schedule_strategy}"
        assert manual_schedule_strategy[-1] == 1, f"manual_schedule_strategy[-1] should be 1."
        
        self.manual_schedule_strategy = manual_schedule_strategy
    ...
    def _generate_schedule(self):
        num_clock = (self.num_mesh + self.num_batch - 1) * 2
        schedules = [[None] * self.num_mesh for _ in range(num_clock)]
        
        
        num_warump_microbatches = [l_n - 1 for l_n in self.manual_schedule_strategy]
        num_microbatches_remaining = [self.num_batch - i for i in num_warump_microbatches]        
        
        
        next_fwd_mb_idx = [0 for _ in range(self.num_mesh)]
        next_bwd_mb_idx = [0 for _ in range(self.num_mesh)]
        next_available_clock = list(range(self.num_mesh))
        finished_bwd_batch_indices = np.zeros(shape=[num_clock, self.num_mesh], dtype=np.int32)
        
        # warm-up phase
        for i in range(self.num_mesh):
            for _ in range(num_warump_microbatches[i]):
                schedules[next_available_clock[i]][i] = (next_fwd_mb_idx[i], i)
                next_available_clock[i] = next_available_clock[i] + 1
                next_fwd_mb_idx[i] = next_fwd_mb_idx[i] + 1
        
        # steady phase
        for i in reversed(range(self.num_mesh)):
            # from the last device to the first
            for _ in range(num_microbatches_remaining[i]):
                # running through all the remaining microbatches
                # forward
                ...
                # backward
                # first, offset the next available clock to the clock
                # when the previous stage has just finished backward of the traget mb.
                if i + 1 < self.num_mesh:  # not the last device
                    # find the next possible backward clock
                ...
                schedules[next_clock][i] = (next_bwd_mb_idx[i], 2 * self.num_mesh - 1 - i)
                finished_bwd_batch_indices[next_clock][i] = next_bwd_mb_idx[i]
                next_bwd_mb_idx[i] = next_bwd_mb_idx[i] + 1
                next_available_clock[i] = next_clock + 1
        ...
        for i in reversed(range(self.num_mesh)):
            ...
            # update status matrix for the last worker
            if i > 0:
                finished_bwd_batch_indices[next_available_clock[i]:num_clock, i] = self.num_batch
        
        # append apply_grad schedules
        scheds = [None] * self.num_mesh
        for stage_idx, worker in self.apply_grad_placement.items():
            scheds[worker] = (self.last_backward_batch_index, stage_idx)
        schedules.append(scheds)
        return schedules
    ...
    @property
    def name(self):
        return "gd_1f1b_{}".format("_".join(map(str, self.manual_schedule_strategy)))
...
pipeline_schedule["gd_1f1b"] = ManualSchedule

from alpa.global_env import global_config
def create_pipeline_schedule(name, dependency, meshes, apply_grad_placement, num_batch, manual_schedule_strategy):
    kwargs = {
    ...
    if global_config.enable_H1F1B:
        ...
        name = "gd_1f1b"
        kwargs["manual_schedule_strategy"] = manual_schedule_strategy

    return pipeline_schedule[name](**kwargs)