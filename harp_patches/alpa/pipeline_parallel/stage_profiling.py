...
def profile_all(stages, compiled_outputs: Sequence[CompileOutput], meshes,
                num_micro_batches, auto_stage_option, profile_results, heter_cluster_idx):
    ...
    if auto_stage_option.use_hlo_cost_model:
        ...
        if global_config.enable_Hetero:
            assert heter_cluster_idx is not None, "heter_cluster_idx should be provided in hetero setting."
            prof_database.load(auto_stage_option.profiling_database_filename[heter_cluster_idx])
            cluster_key = auto_stage_option.cluster_key[heter_cluster_idx]
        else:
            prof_database.load(auto_stage_option.profiling_database_filename)
            cluster_key = auto_stage_option.cluster_key

        prof_result = prof_database.query(cluster_key, meshes[0].shape)
...
def get_merged_stages_memory_stats(
    ...
    for module_id, stage_order in enumerate(module_execution_orders):
        ...
        if module_id == 0 and not inference_mode:
            forward_end_no = stage_no
            intermediate_size = sum(
                size for name, size in env.items()
                if name not in acc_grad_invars and
                   name not in acc_grad_outvars and
                  (name in last_used_stage_no and
                   last_used_stage_no[name] > forward_end_no))
...
def generate_training_stages_1d(layers, accumulator_mapping, acc_grad_invars,
    ...
    for l in tqdm.tqdm(range(0, num_layers)):
        selected_apply_grad_layers = ([] if apply_grad_layers[l] is None else [apply_grad_layers[l]])
...
def generate_training_stages_1d_ZRP(layers, accumulator_mapping, acc_grad_invars,
                                     acc_grad_outvars, apply_grad_layers,
                                     apply_grad_global_info, mesh_id,
                                     autosharding_configs):
    print("- Generate all stage infos (Jaxpr -> HLO)")
    ...
    is_repeated_s = []
    module_id_s = []
    repeated_module_id_s = []
    sub_layer_id_s = []
    assert len(global_config.layer_mark) == num_layers, (
        f"Length of layer_mark {len(global_config.layer_mark)} does not match "
        f"num_layers {num_layers}.")
    for l, l_mark in enumerate(global_config.layer_mark):
        module_id, repeated_module_id, sub_layer_id = (int(x) for x in l_mark.split("_")[1:])
        is_repeated = module_id >= 0
        if not is_repeated:
            assert repeated_module_id == -999, f"repeated_module_id should be -1 for non-repeated layers, but got {repeated_module_id} for layer {l}."
        is_repeated_s.append(is_repeated)
        module_id_s.append(module_id)
        repeated_module_id_s.append(repeated_module_id)
        sub_layer_id_s.append(sub_layer_id)
    ...
    for l in tqdm.tqdm(range(0, num_layers)):
        ...
        if is_repeated_s[l] and repeated_module_id_s[l] > 0:
            key = (module_id_s[l], sub_layer_id_s[l])
            for config_idx, autosharding_config in enumerate(autosharding_configs):
                if autosharding_config is not None:
                    if key in stage_config_cache and config_idx in stage_config_cache[key]:
                        src_stage_indices = stage_config_cache[key][config_idx]
                        stage_indices = (l, mesh_id, config_idx)
                        stage_info_cache[stage_indices] = (src_stage_indices, stage_config)
            continue
        ...
        for config_idx, autosharding_config in enumerate(autosharding_configs):
            if autosharding_config is not None:
                ...
                stages.append(
                    (stage_indices, stage_config, autosharding_config))
                stage_info_cache[stage_indices] = stage_indices
                
                if is_repeated_s[l] and repeated_module_id_s[l] == 0:
                    key = (module_id_s[l], sub_layer_id_s[l])
                    if key not in stage_config_cache:
                        stage_config_cache[key] = {}
                    stage_config_cache[key][config_idx] = stage_indices
                    
    return stages, stage_info_cache
...
def distributed_profile_on_mesh(stages, meshes: Sequence[VirtualPhysicalMesh],
                                ...
                                auto_stage_option, profile_results, heter_cluster_idx):
    ...
    profile_results = profile_all(stages, compiled_outputs, meshes,
                                  ...
                                  profile_results, heter_cluster_idx)
...
def check_profile_results_consistent(stages,
    ...
    for stage_idx, stage_config, _ in stages:
        ...
        for (module_profile_result, 
             module_profile_config) in zip(profile_result.module_profile_results,
                                           stage_config.module_profile_configs):
...
def get_profile_result_from_cache(profile_results, stage_info_cache):
    for key, val in stage_info_cache.items():
        if key not in profile_results:
            if isinstance(val, tuple) and len(val) == 2:
                src_stage_idx, stage_config = val
                if src_stage_idx not in profile_results:
                    continue
                src_profile_result = profile_results[src_stage_idx]
                
                # Reconstruct initial_var_names from target config
                if src_profile_result.initial_var_names:
                    # Assume 1-to-1 mapping with apply_grad_config.invars
                    target_initial_var_names = tuple(str(v) for v in stage_config.apply_grad_config.invars)
                    if len(target_initial_var_names) != len(src_profile_result.initial_var_sizes):
                         logger.warning(f"ZRP: Initial var count mismatch for stage {key}. "
                                        f"Src sizes: {len(src_profile_result.initial_var_sizes)}, "
                                        f"Tgt names: {len(target_initial_var_names)}. Keeping Src names.")
                         target_initial_var_names = src_profile_result.initial_var_names
                else:
                    target_initial_var_names = ()

                # Create new StageProfileResult
                new_profile_result = StageProfileResult(
                    src_profile_result.n_modules,
                    target_initial_var_names,
                    src_profile_result.initial_var_sizes
                )
                new_profile_result.available_memory = src_profile_result.available_memory
                
                # Reconstruct ModuleProfileResults
                for i, src_mod_result in enumerate(src_profile_result.module_profile_results):
                    if src_mod_result is None:
                        continue
                    
                    tgt_mod_config = stage_config.module_profile_configs[i]
                    
                    new_mod_result = ModuleProfileResult(
                        compute_cost=src_mod_result.compute_cost,
                        peak_memory=src_mod_result.peak_memory,
                        temp_buffer_size=src_mod_result.temp_buffer_size,
                        invar_names=tuple(tgt_mod_config.invar_names),
                        outvar_names=tuple(tgt_mod_config.outvar_names),
                        invar_sizes=src_mod_result.invar_sizes,
                        outvar_sizes=src_mod_result.outvar_sizes,
                        donated_invars=tuple(tgt_mod_config.donated_invars),
                        acc_grad_invars_indices=tuple(tgt_mod_config.acc_grad_invars_indices),
                        acc_grad_outvars_indices=tuple(tgt_mod_config.acc_grad_outvars_indices),
                        available_memory=src_mod_result.available_memory
                    )
                    new_profile_result.module_profile_results[i] = new_mod_result
                
                profile_results[key] = new_profile_result
            else:
                profile_results[key] = profile_results[val]
    return profile_results
...
def get_compute_cost(
        ...
        heter_cluster_idx: int = None,
        heter_cluster_flops_ratio: float = None):
    ...
    for mesh_id, submesh in reversed(list(enumerate(submesh_choices))):
        ...
        if global_config.profile_with_whole_ray_cluster:
            if global_config.enable_Hetero:
                hetero_host_ids, hetero_gpu_type, hetero_gpu_num_per_host = get_global_cluster().hetero_info
                whole_cluster_virtual_mesh = get_global_cluster().get_virtual_physical_mesh(
                    hetero_host_ids[heter_cluster_idx], 
                    hetero_gpu_num_per_host[heter_cluster_idx]
                )
            else:
                whole_cluster_virtual_mesh = get_global_cluster().get_virtual_physical_mesh()
        ...
        if auto_stage_option.layer_profile_mode == "composition":
            if global_config.enable_ZRP:
                raise NotImplementedError("2D profiling is not supported with ZRP yet.")
            ...
            if global_config.enable_ZRP:
                stages, stage_cache_info = generate_training_stages_1d_ZRP(
        ...
        if global_config.enable_ZRP:
            log_str = f"(enabling ZRP) -> {len(stages)}/{len(stage_cache_info)}"
            profile_results = get_profile_result_from_cache(profile_results, stage_cache_info)            
        else:
            log_str = f"(disabling ZRP) -> {len(stages)}"
        print(f"Profiling({log_str}) for submesh {mesh_id} {submesh} takes {toc - tic:.2f} seconds")
        print("-" * 50)


    if auto_stage_option.layer_profile_mode == "composition":
        assert not global_config.enable_ZRP, "2D profiling is not supported with ZRP yet."
        (compute_cost,
    ...
    debug_print_max_n_succ_stages(max_n_succ_stages, 6)
...
def debug_print_max_n_succ_stages(max_n_succ_stages, tail_length=10):
    num_layers = max_n_succ_stages.shape[0]
    num_submeshes = max_n_succ_stages.shape[2]
    num_configs = max_n_succ_stages.shape[3]

    iter = 0
    print(f"\n[Debug] max_n_succ_stages content (Shape: {max_n_succ_stages.shape}):")
    for start in range(num_layers):
        for end in range(start, num_layers):
            valid_entries = []
            for sm in range(num_submeshes):
                for cfg in range(num_configs):
                    val = max_n_succ_stages[start, end, sm, cfg]
                    if val != -1:
                        valid_entries.append(f"(sm={sm}, cfg={cfg}): {val}")
            
            if valid_entries:
                print(f"  Stage Layers [{start}:{end}] \t-> " + ", \t".join(valid_entries))
                iter += 1
                if iter >= tail_length:
                    print(f"  ... (truncated, total valid entries: {iter})")
                    return