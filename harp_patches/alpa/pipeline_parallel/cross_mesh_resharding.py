...
class SymbolicBroadcastReshardingTask(ReshardingTask):
    ...
    def __init__(self, task_spec, collective_group, src_mesh, dst_mesh):
        ...
        if global_config.enable_H1F1B:
            self.use_ib = global_config.resharding_use_ib[src_mesh.mesh_id][dst_mesh.mesh_id]
            print(f"Task ( {src_mesh.mesh_id} -> {dst_mesh.mesh_id} ) use IB: {self.use_ib}")     
            # self.use_ib = True
            # print(f"> WARNING!! Force use IB: True")
        else:
            self.use_ib = False
            print(f"\033[1;31m> [Homogeneous] Task ( {src_mesh.mesh_id} -> {dst_mesh.mesh_id} )Force use_ib = {self.use_ib}\033[0m")
            ...
            for worker, devices_info in (
                ...
                task_dones.append(
                    worker.init_broadcast_communicator.remote(
                        ...
                        self.use_ib, NCCL_MAX_CTAS=global_config.NCCL_MAX_CTAS_CROSS_RESHARDING))
            ray.get(task_dones)
