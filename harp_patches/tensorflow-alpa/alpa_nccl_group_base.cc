...
Status CommGroup::NcclCreateCommunicators(
    ...
    bool is_ib, int NCCL_MAX_CTAS) {
#if XLA_ENABLE_XCCL
  int n_devices = device_global_ranks.size();
  CHECK_EQ(n_devices, device_ids.size());
  // Start: Edited by harp
  if (NCCL_MAX_CTAS < -1) {
    return InvalidArgument("NCCL_MAX_CTAS must be >= -1, but got %d", NCCL_MAX_CTAS);
  }

  ncclConfig_t nccl_config = NCCL_CONFIG_INITIALIZER;
  nccl_config.netName = is_ib ? "IB" : "Socket";

  if (NCCL_MAX_CTAS != -1) {
    nccl_config.maxCTAs = NCCL_MAX_CTAS;
  } else {
    printf("NCCL_MAX_CTAS is -1, using NCCL default configuration.\n");
  }
  // End: Edited by harp
  ...
  for (int i = 0; i < n_devices; i++) {
    ...
    // XLA_CUDA_RETURN_IF_ERROR(ncclCommInitRank(comm.get(), world_size, nccl_uid, rank));  // Edited by harp
    XLA_CUDA_RETURN_IF_ERROR(ncclCommInitRankConfig(comm.get(), world_size, nccl_uid, rank, &nccl_config));  // Edited by harp
  }
