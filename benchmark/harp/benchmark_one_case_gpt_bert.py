...
from util import compute_gpt_parameter_count, compute_gpt_tflops_total
...
def compute_gpt_bert_statistics(benchmark_case, latencies):
    batch_size = benchmark_case.batch_size
    (seq_len, hidden_size, num_layers, num_heads,
     vocab_size) = benchmark_case.model_config
    use_remat = benchmark_case.parallel_args.use_remat

    tflops = compute_gpt_tflops_total(batch_size,
                                seq_len,
                                num_layers,
                                hidden_size,
                                vocab_size,
...
def benchmark_gpt_bert_3d_internal(model_type,
    ...
    # Connect to the cluster, split experiment sub cluster from the global cluster
    if enable_hetero:
        virtual_mesh = []
        host_ids_global, gpu_type_global, num_gpu_per_hosts_global = get_global_cluster().hetero_info ## global_cluster
        assert len(host_ids_global) == len(num_hosts), "num_hosts does not match global hetero cluster info"
        for i in range(len(gpu_type_global)):
            assert len(host_ids_global[i]) >= num_hosts[i], f"num_hosts required is {num_hosts[i]}, but got {len(host_ids_global[i])} in global hetero cluster info"
            assert num_gpu_per_hosts_global[i] >= num_devices_per_host[i], f"num_devices_per_host required is {num_devices_per_host[i]}, but got {num_gpu_per_hosts_global[i]} in global hetero cluster info"
            vm = get_global_cluster().get_virtual_physical_mesh(host_ids=host_ids_global[i][:num_hosts[i]],
                                                                num_devices_per_host=num_devices_per_host[i])
            virtual_mesh.append(vm)
    else:
    ...
    tflops, parameter_count = compute_gpt_bert_statistics(benchmark_case, latencies)
